...
def parseObjFunctionParams(experiment, parseStrategy="MAXGBP"): # input is an experiment
    ...
    for line in reversed(outF.readlines()):
        ...
        if "tefreqs:, " in line and "k index" not in line:
            ...
            bandwidths[kPoint] = float(bandInfo[band + 5]) # should be consistent across iterations
            # print("tefreqs", kPoint, bandwidths[kPoint]) # sanity check

        # 2 check for group velocity
        if "tevelocity:, " in line:
            groupVelocityLine = line.split(",")
            kPoint = int(groupVelocityLine[1].replace(" ",""))
            # print("tevelocity", kPoint) # sanity check
...
def extractFOM(experiment, lossMap, lossContrastMap, groupIndexMap, bandwidths, parseStrategy):
    ...
    if ng0_index == 0:
        print("Insufficient Group Index for solution")
